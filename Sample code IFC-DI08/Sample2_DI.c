...
//	Project				:Interface Free Controller-Digital Input Card (IFC-DI08)
//	Project description	:Sample2 for IFC-DI08 with Control Panel
//  Date				: 21 April 2008
//
//=====================================================================================
...
//=====================================================================================
#define add_di1 		0b000011	// Please make sure the communication address on 
									// IFC-DI08 is compatible with the address 
...
void main(void)
{
	...
	unsigned short temp1,temp2;	// define 16-bits local variable
	...
	while(1)
	{
		...
		//--------------------------------------------------------------------------------
		// This program is mainly for slave card,DI08 with Control Panel Card, IFC-CP04.
		...
		// on master card, MB00. 
		// When signal detected on input device connected to Digital Input port on DI08,
		// the buzzer and LED on MB00 will be activated/deactivated base on the program.
		// The status of digital input will be display on CP04
		...
		{	
			...
			while(1)
			{
				...
				cp1_str("Demo1:Testing on");		// send message to be displayed by calling function cp1_str
				cp1_goto(1,0);						// set the position of message to be displayed
				cp1_str("DI1,DI2,DI3&DI4");			// send message to be displayed by calling function cp1_str		
				if(di_read(add_di1, 1)==0)			// If DI 1 detected signal
				{
					cp1_goto(0,0);					// set the position of message to be displayed
					cp1_str("Signal Detected ");	// send message to be displayed by calling function cp1_str
					cp1_goto(1,0);					// set the position of message to be displayed
					cp1_str("     on DI1     ");	// send message to be displayed by calling function cp1_str
				...
				if(di_read(add_di1,2)==0)			// If DI 2 detected signal
				{
					...
					cp1_str("Signal Detected ");	// send message to be displayed by calling function cp1_str
					cp1_goto(1,0);					// set the position of message to be displayed
					cp1_str("     on DI2     ");	// send message to be displayed by calling function cp1_str
				...
				if((di_read(add_di1, 3)||di_read(add_di1, 4))==0)	// If DI 3 and DI 4 detected signal
				{
					...
					cp1_str("Signal Detected ");	// send message to be displayed by calling function cp1_str
					cp1_goto(1,0);					// set the position of message to be displayed
					cp1_str(" on DI3 and DI4 ");	// send message to be displayed by calling function cp1_str
					led=0xff;						// All LED ON 
					while(1);						// infinity loop					
				}
			}
		...
		if(cp1_sw(2)==0)							// Test whether SW2 on IFC-CP04 is pressed
		{
			...
			delay(50000);							// delay for buzzer ON time
			buzzer = 0;								// OFF buzzer
			di_c1con(add_di1,1);					// Enable Counter 1
			di_c2con(add_di1,1);					// Enable Counter 2
			...
			while(1)
			{
				...
				// user need to press reset button on MB00 or CP04 to exit from the mode
				cp1_goto(0,0);						// set the position of message to be displayed
				cp1_str("Demo2:Counter");			// send message to be displayed by calling function cp1_str
				temp1=di_c1val(add_di1);			// Send value of Counter1 to temp1
				cp1_goto(1,0);						// set the position of message to be displayed
				cp1_str("C1=");						// send message to be displayed by calling function cp1_str
				cp1_goto(1,3);						// set the position of message to be displayed
				cp1_dec(temp1,4);					// send value of counter1 to be displayed by calling function cp1_dec
				temp2=di_c2val(add_di1);			// Send value of Counter2 to temp2
				cp1_goto(1,9);						// set the position of message to be displayed
				cp1_str("C2=");						// send message to be displayed by calling function cp1_str
				cp1_goto(1,12);						// set the position of message to be displayed
				cp1_dec(temp2,4);					// send value of counter2 to be displayed by calling function cp1_dec
			}
		}
		...
		if(cp1_sw(3)==0)			   				// Test whether SW3 on IFC-CP04 is pressed
		{
			...
			while(1)
			{
				...
				// user need to press reset button on MB00 or CP04 to exit from the mode
				cp1_goto(0,0);						// set the position of message to be displayed
				cp1_str("Demo3:DI&Counter");		// send message to be displayed by calling function cp1_str
				temp1=di_c1val(add_di1);			// Send value of Counter1 to temp1
				cp1_goto(1,2);						// set the position of message to be displayed
				cp1_str("C1=");						// send message to be displayed by calling function cp1_str
				cp1_goto(1,5);						// set the position of message to be displayed
				cp1_dec(temp1,3);					// send value of counter1 to be displayed by calling function cp1_dec
				temp2=di_c2val(add_di1);			// Send value of Counter2 to temp2
				cp1_goto(1,9);						// set the position of message to be displayed
				cp1_str("C2=");						// send message to be displayed by calling function cp1_str
				cp1_goto(1,12);						// set the position of message to be displayed
				cp1_dec(temp2,3);					// send value of counter2 to be displayed by calling function cp1_dec
				if(di_read(add_di1,1)==0)			// If DI 1 detected signal
				{									// Display 1 and value of counter1 on LCD
					cp1_goto(1,0);
					cp1_char('1');
				
				}	
				if(di_read(add_di1,2)==0)			// If DI 2 detected signal
				{									// Display 2 on LCD and value of counter2 on LCD
					cp1_goto(1,0);
					cp1_char('2');
				}										
				if(di_read(add_di1,3)==0)			// If DI 3 detected signal
				{									// Display 3 on LCD and clear counter 1 and 2
					cp1_goto(1,0);
					cp1_char('3');
					di_c1clr(add_di1);
					di_c2clr(add_di1);
				}	
				if(di_read(add_di1,4)==0)			// If DI 4 detected signal
				{									// Display 4 on LCD
					cp1_goto(1,0);
					cp1_char('4');
					
				}
				if(di_read(add_di1,5)==0)			// If DI 5 detected signal
				{									// Display 5 on LCD
					cp1_goto(1,0);
					cp1_char('5');
				
				}
				if(di_read(add_di1,6)==0)			// If DI 6 detected signal
				{									// Display 6 on LCD
					cp1_goto(1,0);
					cp1_char('6');
				}		
				if(di_read(add_di1,7)==0)			// If DI 7 detected signal
				{									// Display 7 on LCD
					cp1_goto(1,0);
					cp1_char('7');
				}
				if(di_read(add_di1,8)==0)			// If DI 8 detected signal
				{									// Display 8 on LCD
					cp1_goto(1,0);
					cp1_char('8');
				}
		...
		if(cp1_sw(4)==0)			   				// Test whether SW4 on IFC-CP04 is pressed
		{
			led4 = 1;								// turn ON LED3 on MB00 by sending value '1'
			buzzer = 1;								// buzzer ON for first time
			delay(50000);							// delay for buzzer ON time
			buzzer = 0;								// OFF buzzer
			delay(50000);							// delay for buzzer OFF time
			...
			while(1)
			{
				...
				// user need to press reset button on MB00 or CP04 to exit from the mode
				cp1_goto(0,0);						// set the position of message to be displayed
				cp1_str("Demo4:Testing on");			// send message to be displayed by calling function cp1_str	
				cp1_goto(1,0);				// set the position of message to be displayed
				cp1_str("   ALL Input   ");	// send message to be displayed by calling function cp1_str
				if(di_read(add_di1,0)==0)				// if ALL digital input detected signal
				{
					while(1)
					{
						...
						cp1_str("Signal Detected ");			// send message to be displayed by calling function cp1_str
						cp1_goto(1,0);				// set the position of message to be displayed
						cp1_str("  on ALL Input  ");	// send message to be displayed by calling function cp1_str
						led=0x0f;		
						delay(50000);
						led=0xf0;		
						delay(50000);	
